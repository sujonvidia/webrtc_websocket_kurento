...
var getSubtasklist = (data, callback) =>{
    var query = {
        activity_parent: { $eq: models.uuidFromString(data) },
    };
...
var updateActivity = (clusteringkey,userid,acti,type,callback) =>{
    if(type == 'pin'){
        models.instance.Activity.update({ activity_id: models.uuidFromString(acti), activity_created_at: clusteringkey },{activity_pinned: { $add: [userid] },}, update_if_exists,function (err) {
            if (err) {
                throw err;
                callback({"msg":"fail"});
            } else {
        ...
        models.instance.Activity.update({ activity_id: models.uuidFromString(acti), activity_created_at: clusteringkey  },{activity_pinned: { $remove: [userid] },}, update_if_exists,function (err) {
            if (err) {
                throw err;
                callback({"msg":"fail"});
            } else {
        ...
        models.instance.Activity.update({ activity_id: models.uuidFromString(acti), activity_created_at: clusteringkey },{activity_has_flagged: { $add: [userid] },}, update_if_exists,function (err) {
            if (err) {
                throw err;
                callback({"msg":"fail"});
            } else {
        ...
        models.instance.Activity.update({ activity_id: models.uuidFromString(acti), activity_created_at: clusteringkey  },{activity_has_flagged: { $remove: [userid] },}, update_if_exists,function (err) {
            if (err) {
                throw err;
                callback({"msg":"fail"});
            } else {
        ...
        models.instance.Activity.update({ activity_id: models.uuidFromString(acti), activity_created_at: clusteringkey  },{activity_title: userid}, update_if_exists,function (err) {
            if (err) {
                throw err;
                callback({"msg":"fail"});
            } else {
      ...
      models.instance.Checklist.update({ checklist_activity_id: models.uuidFromString(clusteringkey), checklist_id: models.uuidFromString(acti) },{checklist_title: userid}, update_if_exists,function (err) {
          if (err) {
              throw err;
              callback({"msg":"fail"});
          } else {
        ...
        models.instance.Activity.update({ activity_id: models.uuidFromString(acti), activity_created_at: clusteringkey  },{activity_participants: { $add: [userid] }}, update_if_exists,function (err) {
            if (err) {
                throw err;
                callback({"msg":"fail"});
            } else {
        ...
        models.instance.Activity.update({ activity_id: models.uuidFromString(acti), activity_created_at: clusteringkey }, { activity_accepted: { $add: [userid] } }, update_if_exists, function (err) {
            if (err) {
                throw err;
                callback({ "msg": "fail" });
            } else {
        ...
        models.instance.Activity.update({ activity_id: models.uuidFromString(acti), activity_created_at: clusteringkey  },{activity_participants: { $remove: [userid] }}, update_if_exists,function (err) {
            if (err) {
                throw err;
                callback({"msg":"fail"});
            } else {
        ...
        models.instance.Checklist.update({ checklist_activity_id: models.uuidFromString(clusteringkey), checklist_id: models.uuidFromString(acti)},{ checklist_status:userid }, update_if_exists,function (err) {
            if (err) {
                throw err;
                callback({"msg":"fail"});
            } else {
        ...
        models.instance.Activity.update({ activity_id: models.uuidFromString(acti), activity_created_at: clusteringkey  },{activity_end_time: newDate}, update_if_exists,function (err) {
            if (err) {
                throw err;
                callback({"msg":"fail"});
            } else {
        ...
        models.instance.Activity.update({ activity_id: models.uuidFromString(acti), activity_created_at: clusteringkey  },{activity_workspace: userid}, update_if_exists,function (err) {
            if (err) {
                throw err;
                callback({"msg":"fail"});
            } else {
        ...
        models.instance.Activity.update({ activity_id: models.uuidFromString(acti), activity_created_at: clusteringkey }, { activity_from: userid["activity_from"], activity_to: userid["activity_to"], activity_has_reminder: userid["activity_reminder"] }, update_if_exists, function (err) {
            if (err) {
                throw err;
                callback({ "msg": "fail" });
            } else {
...
var deleteSubTask = (checklist_id, clusteringkey, callback) => {

    var query_object = {
        activity_id: models.uuidFromString(checklist_id),
        activity_created_at: clusteringkey,
    };
...
var update_activity_participants = (data,callback)=>{
    models.instance.Activity.update({ activity_id: models.uuidFromString(data.activity_id), activity_created_at: data.clusteringkey }, { activity_participants: data.activity_participants }, update_if_exists, function (err) {
        if (err) {
            callback({"msg" : err});
        } else {
            callback({ "msg": "success" });
...
var createSubActivity = (activityType, activityTitle, activityDescription, createdBy, endTime, ecosystem, adminList, todoFrom, todoTo, todoReminder,activity_parent, callback) => {
    var adminAccept = [];
    ...
    // var newDate = endTime[1]+"/"+endTime[0]+"/"+endTime[2];
    // let clusteringkey = new Date().getTime();
    var clusteringkey = new Date( (new Date)*1 - 1000*3600*6 ).getTime();
    var activity_id = models.uuid();
    var activity = new models.instance.Activity({
        activity_id: activity_id,
        activity_type: activityType,
        ...
        activity_is_active: 1,
        activity_start_time: clusteringkey,
        activity_end_time: clusteringkey,
        activity_workspace: ecosystem,
        activity_participants: adminList,
        activity_accepted: adminAccept,
        activity_from: todoFrom,
        activity_to: todoTo,
        activity_has_reminder: todoReminder,
        activity_parent:models.uuidFromString(activity_parent)
    });
    activity.saveAsync().then(function() {
        callback({ status: true, activity_id, clusteringkey: clusteringkey});
    }).catch(function(err) {
...
var updateActivityList=(data,callback)=>{
    // if(data.type == 'co_owner'){
    //     models.instance.Activity.update(
    //         { 
    //             activity_id: models.uuidFromString(data.activity_id), 
    //             activity_created_at: data.clusteringkey  
    //         },{
    //             activity_coowner: data.update_data
    //         },function (err) {
    //         if (err) {
    //             callback({"msg":"fail"});
    //         } else {
    ...
    //             activity_created_at: data.clusteringkey  
    //         },{
    //             activity_observer: data.update_data
    //         },function (err) {
    //         if (err) {
    //             callback({"msg":"fail"});
    //         } else {
    ...
    //             activity_created_at: data.clusteringkey  
    //         },{
    //             activity_assignee: data.update_data
    //         },function (err) {
    //         if (err) {
    //             callback({"msg":"fail"});
    //         } else {
...
var getCompanyIdByUser = (data, callback)=>{
    models.instance.Users.findOne({id:models.uuidFromString(data.user_id)}, function(err, result){
        if(err){
        console.log("This is err",err);
        callback({status: false, err: err});
        }else{

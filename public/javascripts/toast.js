 ...
 */
 ...
 function openTostMsg(conversation_type,msg_from, msg_conv_id, msg_sender_name,msg_sender_img){
    window.location.href = '/alpha2/chat/' + conversation_type + '/' + msg_from + '/' + encodeURI(msg_conv_id) + '/' + encodeURI(msg_sender_name) + '/' + encodeURI(msg_sender_img);
 }
function closeToast(){
  $('.error').remove();
}
    function initToast(conversation_type,msg_from, msg_conv_id, msg_sender_name,msg_sender_img,msg_text) {
      ...
      var suffix = parseInt(coutUnread.match(/\d+/));
    	...
    	var html = '<div class="error">' +
                        ...
                        '<div class="error_inner" onclick="toggleToastdiv()"><img style="width:60px; height:60px; border-radius:50%;" src="'+ file_server + 'room-images-uploads/Photos/'+msg_sender_img+'"></div>' +

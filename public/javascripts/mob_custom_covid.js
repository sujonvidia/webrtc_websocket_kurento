...
$(document).keyup(function(e) {
    if (e.which == 27) {
        closePopUps('#profileSettingPopup');
        closePopUps('#languageSettingPopup');
        $(".down_arrow").removeClass('active');
        $('.profilenavMainSection').fadeOut();
...
function startConversation(e, elm) {
    e.preventDefault();
    e.stopImmediatePropagation();
    student_id = $(elm).attr('student-id');
    // var student_name = $(elm).attr('student-name');
    // var student_role = $(elm).attr('student-role');
    var page_type = $(elm).attr('data-type');
    var date_of = moment().endOf('day').unix();
    if (page_type == 'report') {
        window.open('/covid/mobile_survey/' + page_type + '/' + date_of + '/' + user_id, '_self');
    } else if (page_type == 'scan') {
        window.open('/covid/mobile_survey/' + page_type + '/' + date_of + '/' + user_id, '_self');
    } else if (page_type == 'qr') {
        window.open('/covid/mobile_survey/' + page_type + '/' + date_of + '/' + user_id, '_self');
    } else {
        window.open('/covid/mobile_survey/' + page_type + '/' + date_of + '/' + student_id, '_self');
    }

    // if(student_role == 'Parent User' && user_role == 'Student User'){
    // 	// alert(student_id);
    // 	$('#parentsPopup').css('display', 'flex')
    // 	// $('#parentsPopup').data('student_id',student_id);
...
function closePopProfile(selector) {
    $(selector).hide();
}
...
function profileSetting(e) {
    e.stopPropagation();
    // debugger
    if ($('.profilenavMainSection').is(':visible')) $('.head_container .down_arrow').trigger('click');

    $('#profileSettingPopup').css('display', 'flex');
    $("#profileSettingPopup #cur_user_img").val(user_img);
    if (user_img != "feelix.jpg") {
        $("#view_room_img").attr('src', file_server + 'profile-pic/Photos/' + user_img);
        $("#view_company_img").attr('src', file_server + 'profile-pic/Photos/' + company_img);

    }
    $("#profileSettingPopup #fullname").val(findObjForUser(user_id).fullname);
    $("#profileSettingPopup #username").val(findObjForUser(user_id).email);

    $("#profileSettingPopup #email").val(user_row.email);
    $("#profileSettingPopup #profile_phone_number").val(user_row.phone_optional);
    $("#profileSettingPopup #fname").val(user_row.firstname);
    $("#profileSettingPopup #lname").val(user_row.lastname);

    $("#profileSettingPopup #caller_id").val(window.location.origin + '/call/' + findObjForUser(user_id).conference_id);
    // $("#profileSettingPopup #job_title").val(findObjForUser(user_id).designation);
}

function saveUserProfileFun(key, location) {
    var newimg = $('#cur_user_img').val();

}

function upload_company_img(files) {
    var formData = new FormData();
    ...
    $.ajax({
        ...
        processData: false,
        beforeSend: function() {
            $('#profileSettingPopup #view_company_img').attr('src', file_server + 'common/imgLoader.gif');
        },
        success: function(res) {
            // console.log(14020,res);
            // saveUserProfileFun(res.file_info[0].key, res.file_info[0].location);
            var new_img = res.file_info[0].key;
            var data = {
                    img: new_img,
                    fullname: user_fullname,
                    id: user_id,
                    company_id: company_id,
                    company_name: $('#company_id_name').text()
                }
                // console.log(1404,data)
            if (data.img != '' && data.fullname != '') {
                socket.emit('update_company_profile', data, function(res) {
                    // //debugger
                    company_img = new_img;
                    $('#covid_company_pic').attr('src', file_server + 'profile-pic/Photos/' + new_img);
                    $("#view_company_img").attr('src', file_server + 'profile-pic/Photos/' + new_img);
                })
            }
        },
        error: function(e) {
...
function upload_user_img(files) {
    ...
    $.ajax({
        ...
        beforeSend: function() {
            ...
            $('#box_survey_propic img').attr('src', file_server + 'common/imgLoader.gif');
        },
        ...
        success: function(res) {
            ...
            if (data.img != '' && data.fullname != '') {
                ...
                socket.emit('update_user_profile_covid', data, function(res) {
                    $('.cls_profile_img').attr('src', file_server + 'profile-pic/Photos/' + user_img);
                    $('#roomImage').attr('src', file_server + 'profile-pic/Photos/' + user_img);
                    $('#box_survey_propic img').attr('src', file_server + 'profile-pic/Photos/' + user_img);
                    $("#studentid_" + user_id).find('.user_profile_pic').attr('src', file_server + 'profile-pic/Photos/' + user_img);
...
// 	//debugger
// 	$('.cls_profile_img').attr('src',file_server+'profile-pic/Photos/'+data.img);
// 	// $.each(user_list,function(k,v){
// 	// 	if(v.id == data.id){

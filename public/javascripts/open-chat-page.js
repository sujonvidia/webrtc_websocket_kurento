...
$.each(allUserdata[0].users, function(ky, va) {
    ...
    definedList += '  <span class="name s-l-def-clas">' + va.fullname + '</span> <span class="designation-name" data-desig="' + va.designation + '">' + va.designation + '</span>';
    definedList += '</li>';
...
$('.suggested-list li').on('click', function() {
    ...
    if (conversation_type === 'personal') {
        socket.emit('o2otoGroup', {
            ...
            if (callBack.status) {
                window.location.href = '/alpha2/chat/group/' + user_id + '/' + encodeURI(callBack.conversation_id) + '/Group/feelix.jpg';
            }
        });
    } else {
        $.ajax({
...
var group_member_li_draw = (name, img, cnvid, userid, desig) => {
    ...
    mldesign += '   <img src="'+ file_server +'profile-pic/Photos/' + img + '" class="member-img">';

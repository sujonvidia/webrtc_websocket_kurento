...
var lightbox_call = () => {
  $('.lightbox').on('click', function(event) {
    if (!$('#lightbox-modal').is(':visible')) {
      ...
      if (src1 == undefined) {
        var src1 = '/images/basicAssets/' + $(event.target).closest('.attach-file').attr('data-filetype') + '.svg';
      ...
      $.each(allimg, function(k, v) {
        ...
        if ($(v).hasClass('attach-file')) {
          var src = '/images/basicAssets/' + $(v).attr('data-filetype') + '.svg';
          is_file = 'isAfile';
...
var statusChange = (title, color) => {
  ...
  if (title == "Cancelled") {
    ...
    $("#" + idName + "Title").css('color', '#F55 !important');
  }

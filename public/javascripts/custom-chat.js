    ...
    if (to == message.msg_from || conversation_id == message.msg_conv_id) {
        ...
        if (typeof attfiles != "undefined") {
            ...
            if (attfiles.audiofile) {
                ...
                per_msg_file_attachment(attfiles.audiofile);
            }
    ...
    if ($('#conversation-container').length) {
        ...
        $(".pin-bar").droppable({
            drop: function(event, ui) {
                ...
                if (drawpin) {
                    var html = '<div class="pin-item"><img src="' + file_server + 'profile-pic/Photos/' + img + '" title="' + name + '" onclick="unpinme(event)" /></div>';
        ...
        socket.emit('sendMessage', {
            ...
            if (filedata.length) {
                ...
                if (filedata[0].audiofile.length) {
                    ...
                    per_msg_file_attachment(filedata[0].audiofile);
                }
            ...
            if (reply_list.status) {
                ...
                $.each(reply_list_data, function(key, row) {
                    ...
                    if (row.attch_audiofile != null) {
                        ...
                        per_msg_file_attachment(row.attch_audiofile);
                    }
...
var draw_typing_indicator = (add_remove, img, name) => {
    if (add_remove) {
        if ($('.typing-indicator').length < 1) {
            ...
            html += '<div class="profile-picture">';
            html += '<img src="' + file_server + 'profile-pic/Photos/' + img + '" alt="' + img + '">';
            html += '</div>';

...
router.get('/:em?', function(req, res, next) {
  if(req.session.login){
    ...
    models.instance.Company.find({$limit:1000}, function(error, data){
      res.render('signup2', { page_title:'Signup  | NEC',title: 'Signup  | NEC', bodyClass: 'centered-form', success: req.session.success, error: req.session.error, has_login: false, company: data, email: req.params.em });
